...
filez.all <- paste0("yzChechnya_",rep(cntz,each=4),"_clea_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_yzChechnya/")]
filez.older <- dir("Output/Output_yzChechnya")[substr(file.mtime(paste0("Output/Output_yzChechnya/",dir("Output/Output_yzChechnya"))),1,10)<mdate]
...
if(length(cntz)>0){
  ...
  mclapply(k0:length(cntz),function(k){print(cntz[k])
    ...
    i <- 1
    if(paste0("yzChechnya_",cntz[k],"_clea_year.RData")%in%filez.old){
      ...
      adm.year <- agg.data
      summary(adm.year)
      save(adm.year,file=paste0("Output/Output_yzChechnya/yzChechnya_",cntz[k],"_clea_year.RData"))
    }
    
    ## Monthly event counts
    disag <- sort(unique(grid.data$KEY))
    i <- 2
    if(paste0("yzChechnya_",cntz[k],"_clea_month.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
      ...
      adm.month <- agg.data
      save(adm.month,file=paste0("Output/Output_yzChechnya/yzChechnya_",cntz[k],"_clea_month.RData"))
    }
    
    ## Weekly event counts
    disag <- sort(unique(grid.data$KEY))
    i <- 2
    if(paste0("yzChechnya_",cntz[k],"_clea_week.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
      ...
      adm.week <- agg.data
      save(adm.week,file=paste0("Output/Output_yzChechnya/yzChechnya_",cntz[k],"_clea_week.RData"))
    }
    
    ## Daily event counts
    disag <- sort(unique(grid.data$KEY))
    i <- 2
    if(paste0("yzChechnya_",cntz[k],"_clea_day.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
      ...
      adm.day <- agg.data
      save(adm.day,file=paste0("Output/Output_yzChechnya/yzChechnya_",cntz[k],"_clea_day.RData"))
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("yzChechnya_",rep(cntz,each=4*3),"_adm",rep(0:2,each=4),"_",rep(c("year","month","week","day"),3),".RData")
...
for(a in 0:2){print(a)
  ...
  if(!((a==1)&(cntz[k]%in%noadm1))){
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      i <- 1
      if(paste0("yzChechnya_",cntz[k],"_adm",admz,"_year.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
      ...
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("yzChechnya_",cntz[k],"_adm",admz,"_month.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
      ...
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("yzChechnya_",cntz[k],"_adm",admz,"_week.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
      ...
      if(admz!=2){
        disag <- sort(unique(gadm.data$KEY))
        i <- 2
        if(paste0("yzChechnya_",cntz[k],"_adm",admz,"_day.RData")%in%filez.old){
        agg.sub <- lapply(1:length(disag),function(i){
          sub <- subdata[subdata$KEY%in%disag[i],]
          sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
          sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("yzChechnya_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_yzChechnya/")]
filez.older <- dir("Output/Output_yzChechnya")[substr(file.mtime(paste0("Output/Output_yzChechnya/",dir("Output/Output_yzChechnya"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
disag <- sort(unique(grid.data$KEY))
head(subdata)
if(paste0("yzChechnya_",cntz[k],"_priogrid_year.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
...
disag <- sort(unique(grid.data$KEY))
i <- 2
if(paste0("yzChechnya_",cntz[k],"_priogrid_month.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
...
disag <- sort(unique(grid.data$KEY))
i <- 2
if(paste0("yzChechnya_",cntz[k],"_priogrid_week.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
...
disag <- sort(unique(grid.data$KEY))
i <- 2
if(paste0("yzChechnya_",cntz[k],"_priogrid_day.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
