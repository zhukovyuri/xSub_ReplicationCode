...
# List files
filez <- dir("Output/Output_ESOCMexicoDrugRelatedMurders/Events")
filez <- filez[grep("Events",filez)]
cntz <- gsub("ESOCMexicoDrugRelatedMurders_Events_|.RData","",filez)
...
filez.all <- paste0("ESOCMexicoDrugRelatedMurders_",rep(cntz,each=4),"_clea_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ESOCMexicoDrugRelatedMurders/")]
filez.older <- dir("Output/Output_ESOCMexicoDrugRelatedMurders")[substr(file.mtime(paste0("Output/Output_ESOCMexicoDrugRelatedMurders/",dir("Output/Output_ESOCMexicoDrugRelatedMurders"))),1,10)<mdate]
...
# Load events
load("Output/Output_ESOCMexicoDrugRelatedMurders/Events/ESOCMexicoDrugRelatedMurders_Events_MEX.RData")
...
i <- 1
if(paste0("ESOCMexicoDrugRelatedMurders_",cntz[k],"_clea_year.RData")%in%filez.old){
...
adm.year <- agg.data
summary(adm.year)
save(adm.year,file=paste0("Output/Output_ESOCMexicoDrugRelatedMurders/ESOCMexicoDrugRelatedMurders_",cntz[k],"_clea_year.RData"))
}
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("ESOCMexicoDrugRelatedMurders_",rep(cntz,each=4*3),"_adm",rep(0:2,each=4),"_",rep(c("year","month","week","day"),3),".RData")
...
# Load events
load(paste0("Output/Output_ESOCMexicoDrugRelatedMurders/Events/ESOCMexicoDrugRelatedMurders_Events_",cntz[k],".RData",sep=""))
names(events) <- toupper(names(events))
events <- events[is.finite(as.numeric(as.character(events$LONG)))&is.finite(as.numeric(as.character(events$LAT))),]
coords <- events[,c("LONG","LAT")]
for(m in 1:2){coords[,m] <- as.numeric(as.character(coords[,m]))}
...
datez <- seq(as.Date("1900-01-01"), as.Date(Sys.Date()), by="days")
ticker <-data.frame(DATE=gsub("-","",datez),TID=1:length(datez),WID=rep(1:length(datez),each=7)[1:length(datez)],YRMO=substr(gsub("-","",datez),1,6),YEAR=substr(gsub("-","",datez),1,4))
ticker <- ticker[as.character(ticker$YEAR) > 2005 & as.character(ticker$YEAR) < 2012,]
head(ticker)
tail(ticker)
head(events)
...
for(a in 0:2){print(a)
  ...
  if(!((a==1)&(cntz[k]%in%noadm1))){
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      i <- 2
      if(paste0("ESOCMexicoDrugRelatedMurders_",cntz[k],"_adm",admz,"_year.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="YEAR",by.y="DATE")
        sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
      ...
      adm.year <- agg.data
      summary(adm.year)
      save(adm.year,file=paste0("Output/Output_ESOCMexicoDrugRelatedMurders/ESOCMexicoDrugRelatedMurders_",cntz[k],"_adm",admz,"_year.RData"))
      }
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("ESOCMexicoDrugRelatedMurders_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ESOCMexicoDrugRelatedMurders/")]
filez.older <- dir("Output/Output_ESOCMexicoDrugRelatedMurders")[substr(file.mtime(paste0("Output/Output_ESOCMexicoDrugRelatedMurders/",dir("Output/Output_ESOCMexicoDrugRelatedMurders"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
disag <- sort(unique(grid.data$KEY))
head(subdata)
if(paste0("ESOCMexicoDrugRelatedMurders_",cntz[k],"_priogrid_year.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="YEAR",by.y="DATE")
  sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
...
adm.year <- agg.data
summary(adm.year)
save(adm.year,file=paste0("Output/Output_ESOCMexicoDrugRelatedMurders/ESOCMexicoDrugRelatedMurders_",cntz[k],"_priogrid_year.RData"))
}
