...
beissinger.list <- lapply(1:length(disag),function(j){print(j)
  ...
  sub0 <- data.frame(SOURCE=paste0("BeissingerUkraine"),CONFLICT=countrycode(cntz,"iso3c","country.name"),COWN=countrycode(cntz,origin = "iso3c",destination = "cown"),COWC=countrycode(cntz,origin = "iso3c",destination = "cowc"),ISO3=countrycode(cntz,origin = "iso3c",destination = "iso3c"),DATE=sub.datez,LAT=sub.lat,LONG=sub.long,GEOPRECISION=sub.precis,TIMEPRECISION=sub.tprecis)
  sub0
  head(sub0)
  ...
  save(events0,file=paste0("Output/Output_BeissingerUkraine/Events/EventType/BeissingerUkraine_EventType_",countrycode(disag[j],origin = "iso3c",destination = "iso3c"),".RData"))  
  events
