...
# List files
filez <- dir("Output/Output_SCAD/Events")
filez <- filez[grep("Events",filez)]
cntz <- gsub("SCAD_Events_|.RData","",filez)
cntz <- cntz[cntz!="000"]
...
filez.all <- paste0("SCAD_",rep(cntz,each=4),"_clea_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_SCAD/")]
filez.older <- dir("Output/Output_SCAD")[substr(file.mtime(paste0("Output/Output_SCAD/",dir("Output/Output_SCAD"))),1,10)<mdate]
...
mclapply(k0:length(cntz),function(k){print(cntz[k])
  ...
  # Load events
  load(paste0("Output/Output_SCAD/Events/SCAD_Events_",cntz[k],".RData",sep=""))
  ...
  i <- 1
  if(paste0("SCAD_",cntz[k],"_clea_year.RData")%in%filez.old){
  ...
  adm.year <- agg.data
  summary(adm.year)
  save(adm.year,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_clea_year.RData"))
  }

  ## Monthly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("SCAD_",cntz[k],"_clea_month.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  ...
  adm.month <- agg.data
  save(adm.month,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_clea_month.RData"))
  }

  ## Weekly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("SCAD_",cntz[k],"_clea_week.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  ...
  adm.week <- agg.data
  save(adm.week,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_clea_week.RData"))
  }

  ## Daily event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("SCAD_",cntz[k],"_clea_day.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  ...
  adm.day <- agg.data
  save(adm.day,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_clea_day.RData"))
  }
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("SCAD_",rep(cntz,each=4*3),"_adm",rep(0:2,each=4),"_",rep(c("year","month","week","day"),3),".RData")
...
# lapply(list.of.packages, require, character.only = TRUE)
  ...
  # Dates
  datez <- seq(as.Date("1945-05-09"), as.Date(Sys.Date()), by="days")
  ticker <- data.frame(DATE=gsub("-","",datez),TID=1:length(datez),WID=rep(1:length(datez),each=7)[1:length(datez)],YRMO=substr(gsub("-","",datez),1,6),YEAR=substr(gsub("-","",datez),1,4))
  ticker <- ticker[as.character(ticker$DATE)>=range(as.character(events$DATE),na.rm=T)[1]&as.character(ticker$DATE)<=range(as.character(events$DATE),na.rm=T)[2],]
  head(ticker)
  ...
  for(a in 0:2){print(paste0(cntz[k],"/",a))
    ...
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      i <- 1
      if(paste0("SCAD_",cntz[k],"_adm",admz,"_year.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
      ...
      agg.data <- agg.data[order(agg.data$YSID2),]
      adm.year <- agg.data
      save(adm.year,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_adm",admz,"_year.RData"))
      }
      
      ## Monthly event counts
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("SCAD_",cntz[k],"_adm",admz,"_month.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
      ...
      agg.data <- agg.data[order(agg.data$MSID2),]
      adm.month <- agg.data
      save(adm.month,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_adm",admz,"_month.RData"))
      }
      
      ## Weekly event counts
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("SCAD_",cntz[k],"_adm",admz,"_week.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
      ...
      agg.data <- agg.data[order(agg.data$WSID2),]
      adm.week <- agg.data
      save(adm.week,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_adm",admz,"_week.RData"))
      }
      ...
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("SCAD_",cntz[k],"_adm",admz,"_day.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
        sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
      ...
      agg.data <- agg.data[order(agg.data$TSID2),]
      adm.day <- agg.data
      save(adm.day,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_adm",admz,"_day.RData"))
      }
      }
...
# # Fix errors
# ixp2 <- strsplit(dir("Output/Output_SCAD")[file.size(paste0("Output/Output_SCAD/",dir("Output/Output_SCAD")))<3000],"_")
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("SCAD_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_SCAD/")]
filez.older <- dir("Output/Output_SCAD")[substr(file.mtime(paste0("Output/Output_SCAD/",dir("Output/Output_SCAD"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
mclapply(k0:length(cntz),function(k){print(cntz[k])
  ...
  disag <- sort(unique(grid.data$KEY))
  i <- 1
  if(paste0("SCAD_",cntz[k],"_priogrid_year.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
  ...
  adm.year <- agg.data
  summary(adm.year)
  save(adm.year,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_priogrid_year.RData"))
  }

  ## Monthly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("SCAD_",cntz[k],"_priogrid_month.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
  ...
  agg.data <- agg.data[order(agg.data$MSID2),]
  adm.month <- agg.data
  save(adm.month,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_priogrid_month.RData"))
  }

  ## Weekly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("SCAD_",cntz[k],"_priogrid_week.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
  ...
  agg.data <- agg.data[order(agg.data$WSID2),]
  adm.week <- agg.data
  save(adm.week,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_priogrid_week.RData"))
  }

  ## Daily event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("SCAD_",cntz[k],"_priogrid_day.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
  ...
  agg.data <- agg.data[order(agg.data$TSID2),]
  adm.day <- agg.data
  save(adm.day,file=paste0("Output/Output_SCAD/SCAD_",cntz[k],"_priogrid_day.RData"))
  }
