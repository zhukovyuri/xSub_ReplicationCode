...
# List files
filez <- dir("Output/Output_BeissingerProtest/Events")
filez <- filez[grep("Events",filez)]
cntz <- gsub("BeissingerProtest_Events_|.RData","",filez)
cntz <- cntz[cntz!="000"]
...
filez.all <- paste0("BeissingerProtest_",rep(cntz,each=4),"_clea_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_BeissingerProtest/")]
filez.older <- dir("Output/Output_BeissingerProtest")[substr(file.mtime(paste0("Output/Output_BeissingerProtest/",dir("Output/Output_BeissingerProtest"))),1,10)<mdate]
...
mclapply(k0:length(cntz),function(k){print(cntz[k])
  ...
  # Load events
  load(paste0("Output/Output_BeissingerProtest/Events/BeissingerProtest_Events_",cntz[k],".RData",sep=""))
  ...
  i <- 1
  if(paste0("BeissingerProtest_",cntz[k],"_clea_year.RData")%in%filez.old){
  ...
  adm.year <- agg.data
  summary(adm.year)
  save(adm.year,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_clea_year.RData"))
  }
  
  ## Monthly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("BeissingerProtest_",cntz[k],"_clea_month.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  ...
  adm.month <- agg.data
  save(adm.month,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_clea_month.RData"))
  }
  
  ## Weekly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("BeissingerProtest_",cntz[k],"_clea_week.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  ...
  adm.week <- agg.data
  save(adm.week,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_clea_week.RData"))
  }
  
  ## Daily event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("BeissingerProtest_",cntz[k],"_clea_day.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  ...
  adm.day <- agg.data
  save(adm.day,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_clea_day.RData"))
  }
  ...
  for(a in 0:2){print(a)
    ...
    if(!((a==1)&(cntz[k]%in%noadm1))){
      if(!((a==2)&(cntz[k]%in%noadm2))){
        ...
        i <- 1
        if(paste0("BeissingerProtest_",cntz[k],"_adm",admz,"_year.RData")%in%filez.old){
        agg.sub <- lapply(1:length(disag),function(i){
          sub <- subdata[subdata$KEY%in%disag[i],]
          sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
          sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
        ...
        adm.year <- agg.data
        head(adm.year)
        save(adm.year,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_adm",admz,"_year.RData"))
        }
        
        ## Monthly event counts
        disag <- sort(unique(gadm.data$KEY))
        i <- 2
        if(paste0("BeissingerProtest_",cntz[k],"_adm",admz,"_month.RData")%in%filez.old){
        agg.sub <- lapply(1:length(disag),function(i){
          sub <- subdata[subdata$KEY%in%disag[i],]
          sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
          sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
        ...
        agg.data <- agg.data[order(agg.data$MSID2),]
        adm.month <- agg.data
        save(adm.month,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_adm",admz,"_month.RData"))
        }
        
        ## Weekly event counts
        disag <- sort(unique(gadm.data$KEY))
        i <- 2
        if(paste0("BeissingerProtest_",cntz[k],"_adm",admz,"_week.RData")%in%filez.old){
        agg.sub <- lapply(1:length(disag),function(i){
          sub <- subdata[subdata$KEY%in%disag[i],]
          sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
          sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
        ...
        agg.data <- agg.data[order(agg.data$WSID2),]
        adm.week <- agg.data
        save(adm.week,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_adm",admz,"_week.RData"))
        }
        ...
        if(admz!=2){
          disag <- sort(unique(gadm.data$KEY))
          i <- 2
          if(paste0("BeissingerProtest_",cntz[k],"_adm",admz,"_day.RData")%in%filez.old){
          agg.sub <- lapply(1:length(disag),function(i){
            sub <- subdata[subdata$KEY%in%disag[i],]
            sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
            sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
          ...
          agg.data <- agg.data[order(agg.data$TSID2),]
          adm.day <- agg.data
          save(adm.day,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_adm",admz,"_day.RData"))
          }
        }
        # End adm2 exception
      }
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("BeissingerProtest_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_BeissingerProtest/")]
filez.older <- dir("Output/Output_BeissingerProtest")[substr(file.mtime(paste0("Output/Output_BeissingerProtest/",dir("Output/Output_BeissingerProtest"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
mclapply(k0:length(cntz),function(k){print(paste0(cntz[k]," (",k," of ",length(cntz),")"))
  ...
  disag <- sort(unique(grid.data$KEY))
  head(subdata)
  if(paste0("BeissingerProtest_",cntz[k],"_priogrid_year.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
  ...
  adm.year <- agg.data
  # summary(adm.year)
  save(adm.year,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_priogrid_year.RData"))
  }

  ## Monthly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("BeissingerProtest_",cntz[k],"_priogrid_month.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
  ...
  agg.data <- agg.data[order(agg.data$MSID2),]
  adm.month <- agg.data
  save(adm.month,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_priogrid_month.RData"))
  }
  
  ## Weekly event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("BeissingerProtest_",cntz[k],"_priogrid_week.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
  ...
  agg.data <- agg.data[order(agg.data$WSID2),]
  adm.week <- agg.data
  save(adm.week,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_priogrid_week.RData"))
  }
  
  ## Daily event counts
  disag <- sort(unique(grid.data$KEY))
  i <- 2
  if(paste0("BeissingerProtest_",cntz[k],"_priogrid_day.RData")%in%filez.old){
  agg.sub <- lapply(1:length(disag),function(i){
    sub <- subdata[subdata$KEY%in%disag[i],]
    sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
    sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
  ...
  agg.data <- agg.data[order(agg.data$TSID2),]
  adm.day <- agg.data
  save(adm.day,file=paste0("Output/Output_BeissingerProtest/BeissingerProtest_",cntz[k],"_priogrid_day.RData"))
  }
