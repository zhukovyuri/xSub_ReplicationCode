...
source("Code/step4_merge/step4x_variable_select.R")
...
srzz <- dir("Output")
srzz <- srzz[!srzz%in%c("Maps","Output_ACD","Output_ESOC","Output_Covariates","Output_Weather","OLD")]
srzz <- gsub("Output_","",srzz)
# srzz <- c("ACLED","GED","PITF","SCAD","yzChechnya","yzCaucasus2000","yzLibya","yzUkraine2014","ACD","ABA_Darfur")
...
# for(s in 1:length(srzz)){print(srzz[s])
  ...
  filez <- filez[!grepl("Events|ACLED_v6",filez)]
  
  # Remaining
  filez <- filez[!filez%in%dir("Upload/data_rdata_country/")]
  
  if(substr(srz,1,2)!="yz"){filez <- filez[!grepl("priogrid_day|adm2_day",filez)]}
  # if(substr(srz,1,2)=="yz"){filez <- filez[!grepl("_POL_|_USA_",filez)]}
  
  # if(substr(srz,1,2)!="yz"){filez <- filez[!grepl("priogrid_day|adm2_day|_POL_|_USA_",filez)]}
  # if(substr(srz,1,2)=="yz"){filez <- filez[!grepl("_POL_|_USA_",filez)]}
  # filez <- filez[!filez%in%dir("Upload/data_rdata_country/")]
  ...
  mclapply(i0:length(filez),function(i){print(paste0(filez[i]," (",i," of ",length(filez),")"))
    ...
    # Write to file
    write.csv(as.data.frame(indata), file=(paste0("~/Dropbox2/Dropbox (Zhukov research team)/XSub_upload/data_csv_country/",gsub("RData","csv",filez)[i])),row.names = FALSE)
    save(indata, file=(paste0("Upload/data_rdata_country/",filez[i])))
