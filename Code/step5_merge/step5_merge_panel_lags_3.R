...
if(grepl("Ubuntu 18.04 LTS"
         ,sessionInfo()[[4]])){setwd("~/Dropbox2/Dropbox (Zhukov research team)/XSub/Data/")}
...
srzz[c(1,6:13,25,28:31)]
ix <- c(1,6:13,25,28:31)
s <- ix[1]
...
filez <- filez[!grepl("Events|ACLED_v6|GED_v|adm0",filez)]
filez <- filez[grepl("month",filez)&grepl("grid|adm2|adm1",filez)]

# Remaining
filez <- filez[!filez%in%dir("Upload/data_rdata_panel_lags/")]
filez

# if(substr(srz,1,2)!="yz"){filez <- filez[!grepl("priogrid_day|adm2_day|_POL_|_USA_|POL|USA|Events|week|day|year|adm0",filez)]}
# if(substr(srz,1,2)=="yz"){filez <- filez[!grepl("_POL_|_USA_|Events|day|year|adm0",filez)]}
...
lapply(list.of.packages, require, character.only = TRUE)
  ...
  save(indata1, file=(paste0("Upload/data_rdata_panel_lags/",filez[i])))
