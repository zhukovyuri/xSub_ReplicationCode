...
# yzChechnya.list <- lapply(1:length(disag),function(j){print(j)

# # Open loop, Multi-Core
# ncores <- detectCores()
# yzChechnya.list <- mclapply(1:length(disag),function(j){print(j)
...
cnt <- "RUS"
sub0 <- data.frame(SOURCE=paste0("yzChechnya"),CONFLICT=countrycode(cnt,"iso3c","country.name"),COWN=countrycode(cnt,origin = "iso3c",destination = "cown"),COWC=countrycode(cnt,origin = "iso3c",destination = "cowc"),ISO3=countrycode(cnt,origin = "iso3c",destination = "iso3c"),DATE=sub.datez,LAT=sub.lat,LONG=sub.long)
