...
# mdate <- "2017-01-01"
# filez.old <- dir("Output/Output_ACLED")[substr(file.mtime(paste0("Output/Output_ACLED/",dir("Output/Output_ACLED"))),1,10)<mdate]
...
# cntz <- cntz[cntz!="000"]
# 
# # # Replace older filez only
# # filez.done <- dir("Output/Output_ACLED/")
# # filez.done <- filez.done[!grepl("Events|v6|prio|clea",filez.done)]
# # whenz <- c()
# # o <- 1
# # for(o in 1:length(filez.done)){whenz[o] <- as.character(file.info(paste0("Output/Output_ACLED/",filez.done[o]))$mtime)}
# # cntz <- sort(unique(sapply(strsplit(filez.done[which(as.numeric(as.character(gsub("-","",sapply(strsplit(whenz," "),"[",1))))<20170408)],"_"),"[",2)))
# 
# k0 <- 1; cntz[k0]; k <- k0
# 
# # Fix errors (replace older files)
...
# 
# # # Fix errors
# # ixp2 <- strsplit(dir("Output/Output_ACLED")[file.size(paste0("Output/Output_ACLED/",dir("Output/Output_ACLED")))<3000],"_")
...
# 
#   # Dates
#   datez <- seq(as.Date("1900-01-01"), as.Date("2016-11-30"), by="days")
#   ticker <- data.frame(DATE=gsub("-","",datez),TID=1:length(datez),WID=rep(1:length(datez),each=7)[1:length(datez)],YRMO=substr(gsub("-","",datez),1,6),YEAR=substr(gsub("-","",datez),1,4))
...
# # # Replace older filez only
# # filez.done <- dir("Output/Output_ACLED/")
# # filez.done <- filez.done[grepl("prio",filez.done)]
# # whenz <- c()
# # o <- 1
# # for(o in 1:length(filez.done)){whenz[o] <- as.character(file.info(paste0("Output/Output_ACLED/",filez.done[o]))$mtime)}
# # cntz <- sort(unique(sapply(strsplit(filez.done[which(as.numeric(as.character(gsub("-","",sapply(strsplit(whenz," "),"[",1))))<20170408)],"_"),"[",2)))
