...
# List of files
filez <- dir("Output/Output_FM/Events")
filez <- filez[grep("Events",filez)]
cntz <- gsub("FM_Events_|.RData","",filez)
#cntz <- cntz[cntz!="000"]
cntz
...
# Load events
load(paste0("Output/Output_FM/Events/FM_Events_",cntz[k],".RData",sep=""))
...
for(a in 0:2){print(a)
  ...
  if(!((a==1)&(cntz[k]%in%noadm1))){
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      adm.year <- agg.data
      head(adm.year)
      save(adm.year,file=paste0("Output/Output_FM/FM_",cntz[k],"_adm",admz,"_year.RData"))
      ...
      adm.month <- agg.data
      save(adm.month,file=paste0("Output/Output_FM/FM_",cntz[k],"_adm",admz,"_month.RData"))
      ...
      adm.week <- agg.data
      save(adm.week,file=paste0("Output/Output_FM/FM_",cntz[k],"_adm",admz,"_week.RData"))
      ...
      if(admz!=2){
        ...
        adm.day <- agg.data
        save(adm.day,file=paste0("Output/Output_FM/FM_",cntz[k],"_adm",admz,"_day.RData"))
      }
      # End adm2 exception
    }
  }
...
# Load events
load("Output/Output_FM/Events/FM_Events_FRA.RData")
names(events) <- toupper(names(events))
events <- events[is.finite(as.numeric(as.character(events$LONG)))&is.finite(as.numeric(as.character(events$LAT))),]
coords <- events[,c("LONG","LAT")]
for(m in 1:2){coords[,m] <- as.numeric(as.character(coords[,m]))}
...
agg.data <- agg.data[order(agg.data$YSID2),]
adm.year <- agg.data
save(adm.year,file=paste0("Output/Output_FM/FM_",cntz[k],"_priogrid_year.RData"))
...
agg.data <- agg.data[order(agg.data$MSID2),]
adm.month <- agg.data
save(adm.month,file=paste0("Output/Output_FM/FM_",cntz[k],"_priogrid_month.RData"))
...
agg.data <- agg.data[order(agg.data$WSID2),]
adm.week <- agg.data
save(adm.week,file=paste0("Output/Output_FM/FM_",cntz[k],"_priogrid_week.RData"))
...
# agg.data <- agg.data[order(agg.data$TSID2),]
# adm.day <- agg.data
# save(adm.day,file=paste0("Output/Output_FM/FM_",cntz[k],"_priogrid_day.RData"))
