...
# mdate <- "2017-01-01"
# filez.old <- dir("Output/Output_PITF")[substr(file.mtime(paste0("Output/Output_PITF/",dir("Output/Output_PITF"))),1,10)<mdate]
...
# 
# # # Limit to remaining
# # cntz <- cntz[!cntz%in%na.omit(sapply(ixp, '[', 2)[sapply(ixp, '[', 4)=="day.RData"&sapply(ixp, '[', 3)!="priogrid"])] # no days
# 
# # Fix errors
...
# # Limit to remaining
# # cntz <- cntz[!cntz%in%na.omit(sapply(ixp, '[', 2)[sapply(ixp, '[', 4)=="day.RData"&sapply(ixp, '[', 3)=="priogrid"])] # no days
# # cntz <- cntz[!cntz%in%na.omit(sapply(ixp, '[', 2)[])]
# 
# # Fix errors
# ixp2 <- strsplit(dir("Output/Output_PITF")[file.size(paste0("Output/Output_PITF/",dir("Output/Output_PITF")))<3000],"_")
# ixp2 <- ixp2[sapply(ixp2, '[', 4)=="week.RData"]
