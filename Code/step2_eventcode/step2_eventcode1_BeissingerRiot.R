...
# data$ISO3 <- NA
# data[data$REPUBLIC_RENAME == "Armenia", ][, "ISO3"] <- "ARM"
# data[data$REPUBLIC_RENAME == "Azerbaijan", ][, "ISO3"] <- "AZE"
# data[data$REPUBLIC_RENAME == "Belarus", ][, "ISO3"] <- "BLR"
# data[data$REPUBLIC_RENAME == "Estonia", ][, "ISO3"] <- "EST"
# data[data$REPUBLIC_RENAME == "Georgia", ][, "ISO3"] <- "GEO"
# data[data$REPUBLIC_RENAME == "Kazakhstan", ][, "ISO3"] <- "KAZ"
# data[data$REPUBLIC_RENAME == "Kyrgyzstan", ][, "ISO3"] <- "KGZ"
# data[data$REPUBLIC_RENAME == "Latvia", ][, "ISO3"] <- "LVA"
# data[data$REPUBLIC_RENAME == "Lithuania", ][, "ISO3"] <- "LTU"
# data[data$REPUBLIC_RENAME == "Moldova", ][, "ISO3"] <- "MDA"
# data[data$REPUBLIC_RENAME == "Russia", ][, "ISO3"] <- "RUS"
# data[data$REPUBLIC_RENAME == "Tajikistan", ][, "ISO3"] <- "TJK"
# data[data$REPUBLIC_RENAME == "Turkmenistan", ][, "ISO3"] <- "TKM"
# data[data$REPUBLIC_RENAME == "Ukraine", ][, "ISO3"] <- "UKR"
# data[data$REPUBLIC_RENAME == "Uzbekistan", ][, "ISO3"] <- "UZB"
...
beissinger.list <- lapply(1:length(disag),function(j){print(j)
  ...
  sub0 <- data.frame(SOURCE=paste0("BeissingerRiot"),CONFLICT=countrycode(cntz,"iso3c","country.name"),COWN=countrycode(cntz,origin = "iso3c",destination = "cown"),COWC=countrycode(cntz,origin = "iso3c",destination = "cowc"),ISO3=countrycode(cntz,origin = "iso3c",destination = "iso3c"),DATE=sub.datez,LAT=sub.lat,LONG=sub.long)
  sub0
  head(sub0)
