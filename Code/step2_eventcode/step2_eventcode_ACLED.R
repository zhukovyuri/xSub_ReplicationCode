...
acled.list <- mclapply(1:length(disag),function(j){print(j)
  ...
  # Save 
  save(events,file=paste0("Output/Output_ACLED/Events",init.type,"/ACLED_Events_",countrycode(disag[j],origin = "iso3c",destination = "iso3c"),".RData"))  
  events0 <- events0[,names(events0)[!names(events0)%in%names(events)]]
  save(events0,file=paste0("Output/Output_ACLED/Events",init.type,"/EventType/ACLED_EventType_",countrycode(disag[j],origin = "iso3c",destination = "iso3c"),".RData"))  
  events
