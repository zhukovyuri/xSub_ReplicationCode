...
beissinger.list <- lapply(1:length(disag),function(j){print(j)
  ...
  sub0 <- data.frame(SOURCE=paste0("BeissingerProtest"),CONFLICT=countrycode(cntz,"iso3c","country.name"),COWN=countrycode(cntz,origin = "iso3c",destination = "cown"),COWC=countrycode(cntz,origin = "iso3c",destination = "cowc"),ISO3=countrycode(cntz,origin = "iso3c",destination = "iso3c"),DATE=sub.datez,LAT=sub.lat,LONG=sub.long)
  sub0
  head(sub0)
