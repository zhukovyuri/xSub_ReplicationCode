...
filez.all <- paste0("ABADarfur_",rep(cntz,each=4*3),"_adm",rep(0:2,each=4),"_",rep(c("year","month","week","day"),3),".RData")
filez.all <- filez.all[!grepl(paste0(c(paste0(noadm1,"_adm1"),paste0(noadm2,"_adm2")),collapse="|"),filez.all)]
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ABADarfur/")]
filez.older <- dir("Output/Output_ABADarfur")[substr(file.mtime(paste0("Output/Output_ABADarfur/",dir("Output/Output_ABADarfur"))),1,10)<mdate]
...
# Load events
load(paste0("Output/Output_ABADarfur/Events/ABADarfur_Events_",cntz[k],".RData",sep=""))
...
for(a in 0:2){print(a)
  ...
  if(!((a==1)&(cntz[k]%in%noadm1))){
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      adm.year <- agg.data
      head(adm.year)
      save(adm.year,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_adm",admz,"_year.RData"))
      ...
      adm.month <- agg.data
      save(adm.month,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_adm",admz,"_month.RData"))
      ...
      adm.week <- agg.data
      save(adm.week,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_adm",admz,"_week.RData"))
      ...
      if(admz!=2){
        ...
        adm.day <- agg.data
        save(adm.day,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_adm",admz,"_day.RData"))
      }
      # End adm2 exception
    }
  }
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("ABADarfur_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ABADarfur/")]
filez.older <- dir("Output/Output_ABADarfur")[substr(file.mtime(paste0("Output/Output_ABADarfur/",dir("Output/Output_ABADarfur"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
for(k in k0:length(cntz)){print(cntz[k])
  ...
  adm.year <- agg.data
  head(adm.year)
  save(adm.year,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_priogrid_year.RData"))
  ...
  agg.data <- agg.data[order(agg.data$MSID2),]
  adm.month <- agg.data
  save(adm.month,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_priogrid_month.RData"))
  ...
  agg.data <- agg.data[order(agg.data$WSID2),]
  adm.week <- agg.data
  save(adm.week,file=paste0("Output/Output_ABADarfur/ABADarfur_",cntz[k],"_priogrid_week.RData"))
  ...
  # agg.data <- agg.data[order(agg.data$TSID2),]
  # adm.day <- agg.data
  # save(adm.day,file=paste0("Output/Output_ABADarfur/ABADarfur",cntz[k],"_priogrid_day.RData"))
