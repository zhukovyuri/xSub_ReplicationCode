...
load("Input/Events/Beissinger/Soviet Union/Protest_GEO.RData")
...
beissinger.list <- lapply(1:length(disag),function(j){print(j)
  ...
  sub0 <- data.frame(SOURCE=paste0("BeissingerProtest"),CONFLICT=countrycode(cntz,"iso3c","country.name"),COWN=countrycode(cntz,origin = "iso3c",destination = "cown"),COWC=countrycode(cntz,origin = "iso3c",destination = "cowc"),ISO3=countrycode(cntz,origin = "iso3c",destination = "iso3c"),DATE=sub.datez,LAT=sub.lat,LONG=sub.long,GEOPRECISION=sub.precis,TIMEPRECISION=sub.tprecis)
  sub0
  head(sub0)
  ...
  subdata_GOV <- subdata[subdata$VIOLEVEL>0,]
  sub0_GOV <- sub0[subdata$VIOLEVEL>0,]
  ...
  sub0$ACTION_ANY <- 1
  sub0$ACTION_IND <- 0
  sub0$ACTION_DIR <- 1*(subdata$POLINJURED>0|subdata$POLDEATHS>0)
  ...
  if(nrow(sub0_GOV)>0){
    ...
    sub0_GOV$ACTION_IND <- 1*(subdata_GOV$VIOLEVEL>=4)
    sub0_GOV$ACTION_DIR <- 1*(subdata_GOV$VIOLEVEL>0&subdata_GOV$VIOLEVEL<4)
  ...
  events0$ACTION_ARREST <- 1*(subdata[,"ARRESTS"]%in%"Y")*(events$SIDEA_ANY==1)
  ...
  # Save
  save(events,file=paste0("Output/Output_BeissingerProtest/Events/BeissingerProtest_Events_",countrycode(cntz,origin = "iso3c",destination = "iso3c"),".RData"))
  save(events0,file=paste0("Output/Output_BeissingerProtest/Events/EventType/BeissingerProtest_EventType_",countrycode(disag[j],origin = "iso3c",destination = "iso3c"),".RData"))  
  events
...
# save(events,file=paste0("Output/Output_BeissingerProtest/Events/BeissingerProtest_Events_000.RData"))
