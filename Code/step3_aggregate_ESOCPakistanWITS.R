...
# List of files
filez <- dir("Output/Output_ESOCPakistanWITS/Events")
filez <- filez[grep("Events",filez)]
cntz <- gsub("ESOCPakistanWITS_Events_|.RData","",filez)
#cntz <- cntz[cntz!="000"]
cntz
...
filez.all <- paste0("ESOCPakistanWITS_",rep(cntz,each=4*3),"_adm",rep(0:2,each=4),"_",rep(c("year","month","week","day"),3),".RData")
filez.all <- filez.all[!grepl(paste0(c(paste0(noadm1,"_adm1"),paste0(noadm2,"_adm2")),collapse="|"),filez.all)]
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ESOCPakistanWITS/")]
filez.older <- dir("Output/Output_ESOCPakistanWITS")[substr(file.mtime(paste0("Output/Output_ESOCPakistanWITS/",dir("Output/Output_ESOCPakistanWITS"))),1,10)<mdate]
...
# Load events
load(paste0("Output/Output_ESOCPakistanWITS/Events/ESOCPakistanWITS_Events_",cntz[k],".RData",sep=""))
...
for(a in 0:2){print(a)
  ...
  if(!((a==1)&(cntz[k]%in%noadm1))){
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      i <- 1
      if(paste0("ESOCPakistanWITS_",cntz[k],"_adm",admz,"_year.RData")%in%filez.old){
      ...
      adm.year <- agg.data
      head(adm.year)
      save(adm.year,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_adm",admz,"_year.RData"))
      }
      
      ## Monthly event counts
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("ESOCPakistanWITS_",cntz[k],"_adm",admz,"_month.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
      ...
      adm.month <- agg.data
      save(adm.month,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_adm",admz,"_month.RData"))
      }
      
      ## Weekly event counts
      disag <- sort(unique(gadm.data$KEY))
      i <- 2
      if(paste0("ESOCPakistanWITS_",cntz[k],"_adm",admz,"_week.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
      ...
      adm.week <- agg.data
      save(adm.week,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_adm",admz,"_week.RData"))
      }
      ...
      if(admz!=2){
        ...
        adm.day <- agg.data
        save(adm.day,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_adm",admz,"_day.RData"))
        }
      }
      # End adm2 exception
    }
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("ESOCPakistanWITS_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ESOCPakistanWITS/")]
filez.older <- dir("Output/Output_ESOCPakistanWITS")[substr(file.mtime(paste0("Output/Output_ESOCPakistanWITS/",dir("Output/Output_ESOCPakistanWITS"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
# Load events
load("Output/Output_ESOCPakistanWITS/Events/ESOCPakistanWITS_Events_PAK.RData")
names(events) <- toupper(names(events))
events <- events[is.finite(as.numeric(as.character(events$LONG)))&is.finite(as.numeric(as.character(events$LAT))),]
coords <- events[,c("LONG","LAT")]
for(m in 1:2){coords[,m] <- as.numeric(as.character(coords[,m]))}
...
head(subdata)
if(paste0("ESOCPakistanWITS_",cntz[k],"_priogrid_year.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
...
agg.data <- agg.data[order(agg.data$YSID2),]
adm.year <- agg.data
save(adm.year,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_priogrid_year.RData"))
}

## Monthly event counts
disag <- sort(unique(grid.data$KEY))
i <- 2
if(paste0("ESOCPakistanWITS_",cntz[k],"_priogrid_month.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$MSID <- sub$KEY*(10^nchar(max(as.character(ticker$YRMO))))+as.numeric(as.character(sub$YRMO))
...
agg.data <- agg.data[order(agg.data$MSID2),]
adm.month <- agg.data
save(adm.month,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_priogrid_month.RData"))
}

## Weekly event counts
disag <- sort(unique(grid.data$KEY))
i <- 2
if(paste0("ESOCPakistanWITS_",cntz[k],"_priogrid_week.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$WSID <- sub$KEY*(10^nchar(max(ticker$WID)))+sub$WID
...
agg.data <- agg.data[order(agg.data$WSID2),]
adm.week <- agg.data
save(adm.week,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_priogrid_week.RData"))
}

## Daily event counts
disag <- sort(unique(grid.data$KEY))
i <- 2
if(paste0("ESOCPakistanWITS_",cntz[k],"_priogrid_day.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="DATE",by.y="DATE")
  sub$TSID <- sub$KEY*(10^nchar(max(ticker$TID)))+sub$TID
...
agg.data <- agg.data[order(agg.data$TSID2),]
adm.day <- agg.data
save(adm.day,file=paste0("Output/Output_ESOCPakistanWITS/ESOCPakistanWITS_",cntz[k],"_priogrid_day.RData"))
}
