...
filez.all <- paste0("ESOCMexicoHomicide_",rep(cntz,each=4),"_clea_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ESOCMexicoHomicide/")]
filez.older <- dir("Output/Output_ESOCMexicoHomicide")[substr(file.mtime(paste0("Output/Output_ESOCMexicoHomicide/",dir("Output/Output_ESOCMexicoHomicide"))),1,10)<mdate]
...
i <- 1
if(paste0("ESOCMexicoHomicide_",cntz[k],"_clea_year.RData")%in%filez.old){
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("ESOCMexicoHomicide_",rep(cntz,each=4*3),"_adm",rep(0:2,each=4),"_",rep(c("year","month","week","day"),3),".RData")
...
for(a in 0:2){print(a)
  ...
  if(!((a==1)&(cntz[k]%in%noadm1))){
    if(!((a==2)&(cntz[k]%in%noadm2))){
      ...
      i <- 1
      if(paste0("ESOCMexicoHomicide_",cntz[k],"_adm",admz,"_year.RData")%in%filez.old){
      agg.sub <- lapply(1:length(disag),function(i){
        sub <- subdata[subdata$KEY%in%disag[i],]
        sub <- merge(ticker,sub[,],by.x="YEAR",by.y="DATE")
        sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
...
# NEW: Missing and older files
source("Code/step3_aggregate/step3x_mdatez.R")
filez.all <- paste0("ESOCMexicoHomicide_",rep(cntz,each=4),"_priogrid_",c("year","month","week","day"),".RData")
filez.old <- filez.all[!filez.all%in%dir("Output/Output_ESOCMexicoHomicide/")]
filez.older <- dir("Output/Output_ESOCMexicoHomicide")[substr(file.mtime(paste0("Output/Output_ESOCMexicoHomicide/",dir("Output/Output_ESOCMexicoHomicide"))),1,10)<mdate]
filez.older <- intersect(filez.all,filez.older)
filez.old <- union(filez.old,filez.older)
...
disag <- sort(unique(grid.data$KEY))
head(subdata)
if(paste0("ESOCMexicoHomicide_",cntz[k],"_priogrid_year.RData")%in%filez.old){
agg.sub <- lapply(1:length(disag),function(i){
  sub <- subdata[subdata$KEY%in%disag[i],]
  sub <- merge(ticker,sub[,],by.x="YEAR",by.y="DATE")
  sub$YSID <- sub$KEY*(10^nchar(max(as.character(ticker$YEAR))))+as.numeric(as.character(sub$YEAR))
